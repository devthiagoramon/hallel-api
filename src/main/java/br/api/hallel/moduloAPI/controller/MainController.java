...
import br.api.hallel.moduloAPI.exceptions.main.SolicitarCadastroException;
import br.api.hallel.moduloAPI.exceptions.main.SolicitarLoginException;
...
import br.api.hallel.moduloAPI.service.ministerio.ConvidadoEscalaMinisterioService;
...
public class MainController {
    ...
    @Autowired
    private ConvidadoEscalaMinisterioService convidadoEscalaMinisterioService;
    ...
    public ResponseEntity<List<EventosVisualizacaoResponse>> listAllEvento(
        ...
        return ResponseEntity.status(200)
                .body(this.eventosService.listarAllEventos(page,size));
    }

    @GetMapping("/home/evento/{idEvento}/listar")
    public ResponseEntity<EventosPublicResponse> listEventoById(
            @PathVariable String idEvento) {
        return ResponseEntity.status(200)
                .body(this.eventosService.listarEventoPublicById(idEvento));
    }

    @GetMapping("/home/evento/search")
    @Operation(summary = "Pesquisar o evento pelo seu titulo")
    public ResponseEntity<List<EventosVisualizacaoResponse>> searchEventoByTitulo(
            @RequestParam(name="titulo") String titulo) {
        return ResponseEntity.status(200)
                             .body(this.eventosService.listarEventosByName(titulo));
    }
    ...
    public ResponseEntity<List<MinisterioPublicResponse>> listMinisteriosPublic(
            @RequestParam(name = "page",defaultValue = "0") int page,
            @RequestParam(name = "size",defaultValue = "10") int size) {
        return ResponseEntity.ok(this.ministerioService.listMinisterioPublic(page,size));
    }

    @GetMapping("/ministerio/id/{idMinisterio}")
    public ResponseEntity<MinisterioWithCoordsResponse> listMinisteriosPublicById(
    ...
    /**
     * Parte de convidados de uma escala
     */

    @GetMapping("/convidado-escala/{idEscala}")
    @Operation(summary = "Convidados de uma escala",
            description = "Listar os convidados de uma determinada escala pelo id da escala"
    )
    public ResponseEntity<List<ConvidadoEscalaMinisterioUserResponse>> listarConvidadosOfEscala(
            @PathVariable("idEscala") String idEscala) {
        return ResponseEntity.ok()
                .body(this.convidadoEscalaMinisterioService.listConvidadosUserInfos(idEscala));
    }
