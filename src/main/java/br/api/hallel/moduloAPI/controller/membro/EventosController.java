...
public class EventosController {
    ...
    public ResponseEntity<List<EventosVisualizacaoResponse>> listAllEventos() {
        return ResponseEntity.ok()
                             .body(service.listEventosSemDestaqueToVisualizar());
    }

    @GetMapping("/{id}")
    public ResponseEntity<Eventos> listEventoByIdController(
            @PathVariable(value = "id") String id) {
        return ResponseEntity.status(201)
                             .body(service.listarEventoById(id));
    }

    @GetMapping("/{nome}")
    public ResponseEntity<EventoResponse> listEventoByNomeController(
            @PathVariable(value = "nome") String nome) {
        return ResponseEntity.status(201)
                             .body(service.listarEventosByTitulo(nome));
    }


    @GetMapping("/delete/{id}")
    public ResponseEntity<?> deleteEventoById(
    ...
    public Boolean adicionarMembro(
    ...
    public ResponseEntity<List<EventoResponse>> listEventosInscritos(
    ...
    public ResponseEntity<EventoUsuarioVerifyResponse> verificarIdMembro(
    ...
    public ResponseEntity<Boolean> verificarIsInscrito(
    ...
    public ResponseEntity<StatusEntradaEvento> verificarSituacaoMembroEmEvento(
            @RequestParam(value = "idEvento") String idEvento,
            @RequestParam(value = "email") String emailMembro) {
    ...
    public ResponseEntity<ValorEventoResponse> ListValorEvento(
            @PathVariable(value = "id") String idEvento,
            String idMembro) {
        return ResponseEntity.ok()
                             .body(this.service.InformacoesValorEvento(idEvento, idMembro));
    }

    @GetMapping("/localEvento")
    public ResponseEntity<List<LocalEventoResponse>> ListValorEvento() {
        return ResponseEntity.ok().body(this.service.listLocalEvento());
    }
