...
public class MembroService implements MembroInterface {
    ...
    public List<MembroResponse> listAllMembros(Pageable pageable) {
        log.info("Listando membros");
        List<MembroResponse> responseList = new ArrayList<>();

        for (Membro membros : this.repository.findAllByOrderByStatusMembroDescNomeAsc(pageable)) {
            responseList.add(new MembroResponse().toList(membros));
        }
    ...
    public Membro listMembroId(String id) {
        ...
        if (membroOptional.isPresent()) {
            ...
            log.info("Membro com id: '" + id + "' não encontrado");
            return null;
        }
    }
    ...
    public void deleteMembroById(String id) {
        ...
        if (optional.isPresent()) {
            ...
            Membro membro = optional.get();
            String fileName = membro.getFileImageUrl().substring(membro.getFileImageUrl().lastIndexOf("/") + 1);
            try {

                if (bucketService.deleteImageOfBucket(fileName)) {
                    log.info("Imagem '{}' deletada com sucesso do bucket! ", fileName);
                } else {
    ...
    @Override
    public List<MembroResponse> listAllMembrosByName(String name) {
        log.info("Searching members that have {} in name", name);
        return this.repository.findAllByNomeContainingIgnoreCase(name);
    }

    @Override
    public PerfilResponse editarImagePerfilMembro(String membroId,
                                                  MultipartFile image) throws
            IllegalAccessException {
        Optional<Membro> optionalMembro = this.repository.findById(membroId);
        ...
        try {
            String fileImageUrl = "";
            if (oldMembro.getFileImageUrl() != null) {

                fileImageUrl = bucketService.updateImageOfBucket(image, GoogleBucketUtils.getImageName(oldMembro.getId(), Membro.class.getSimpleName()));
            } else {
                fileImageUrl = bucketService.sendImageToBucket(image,  GoogleBucketUtils.getImageName(oldMembro.getId(), Membro.class.getSimpleName()));
            }
            oldMembro.setFileImageUrl(fileImageUrl);

            this.repository.save(oldMembro);
            return new PerfilResponse().toPerfilResponse(oldMembro);
        } catch (IOException e) {
            throw new MembroEditImageException("Não foi possivel editar imagem do membro!");
        }
