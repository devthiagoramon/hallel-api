...
public class AdministradorService implements AdministradorInterface {
    ...
    public AdministradorResponse logarAdministrador(
        ...
        administradorResponse.setValidateCode(verificationCode);

        administrador.setVerificationCode(verificationCode);
        this.repository.save(administrador);
        log.info("Código de verificação (2FA): " + verificationCode);
        if (administrador.getNome() != null && administrador.getEmail() != null &&
                administrador.getEmail().contains("@hallel")) {
    ...
    @Override
    public AuthenticationResponse validateLoginAdm(ConfirmCodeRequest requestDto) {
        Optional<Administrador> optionalAdm = this.repository.findByEmail(requestDto.getEmail());
        if (optionalAdm.isEmpty()) {
            throw new SolicitarLoginException("E-mail inválido.");
        }

        Administrador administrador = optionalAdm.get();

        if (administrador.getVerificationCode() == null || !administrador.getVerificationCode().equals(requestDto.getCodigo())) {
            throw new SolicitarLoginException("Código inválido ou expirado.");
        }
        log.info("ADM ENCONTRADO!");
        ...
        response.setNome(administrador.getNome());
        response.setRoles(administrador.getRoles());
        response.setValidateCode(requestDto.getCodigo());
